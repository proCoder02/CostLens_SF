...
class APIConnection(Base):
    ...
    # ── Custom API fields ────────────────────────────────────────
    base_url = Column(String(500), default="")              # e.g. https://rapexapi.nodexdata.click
    auth_header = Column(String(100), default="X-API-Key")  # header name for the key
    endpoints = Column(JSONB, default=[])                   # e.g. ["/rapex_alerts", "/alerts"]
    cost_per_record = Column(Float, default=0.01)           # price per record returned
